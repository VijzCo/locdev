...
import { FEATURE_KEYS, FEATURE_LABELS } from '@/platform/settings';
...
export function Diagnostics() {
  const { profile, licence, licenceDoc, usage, capabilities, isDevAdmin, effectiveTenantId } =
    useAuth();
  ...
  const account: Row[] = [
    ...
    {
      label: 'Capabilities',
      // A count alone hides the usual cause: a capability list that filtered
      // down to nothing and silently emptied the menu.
      value: capabilities.length === 0 ? 'none — menu will be empty' : capabilities.join(', '),
      tone: capabilities.length === 0 ? 'bad' : 'ok',
    },
    { label: 'Dev admin', value: isDevAdmin ? 'yes' : 'no', tone: isDevAdmin ? 'ok' : undefined },
  ];

  const features: Row[] = FEATURE_KEYS.map((key) => ({
    label: FEATURE_LABELS[key].name,
    value: featureOn(key) ? 'on' : 'off — hidden from customers',
    tone: featureOn(key) ? 'ok' : 'warn',
  }));
  ...
  return (
    <Card>
      <CardBody className="space-y-4 pt-4">
        ...
        />
        <Section title="Areas switched on" rows={features} />
