...
beforeEach(async () => {
  ...
  await env.withSecurityRulesDisabled(async (ctx) => {
    ...
    // Developer access is a document, not a constant in the rules file.
    await setDoc(doc(db, 'devAdmins/vendor1'), { grantedAt: new Date().toISOString() });
...
describe('developer access', () => {
  it('is granted by a devAdmins document', async () => {
  ...
  it('cannot be granted from inside a tenant', async () => {
    // Otherwise any administrator could promote themselves to developer.
    await assertFails(
      setDoc(doc(as('alice'), 'devAdmins/alice'), { grantedAt: new Date().toISOString() }),
    );
  });

  it('still accepts an existing vendorAdmins grant, so a rename cannot lock anyone out', async () => {
    await env.withSecurityRulesDisabled(async (ctx) => {
      await setDoc(doc(ctx.firestore(), 'vendorAdmins/legacy1'), {
        grantedAt: new Date().toISOString(),
      });
      await setDoc(doc(ctx.firestore(), 'users/legacy1'), profile(TENANT));
    });
    await assertSucceeds(getDoc(doc(as('legacy1'), `licenses/${OTHER}`)));
  });
});
