...
if ($count > 0) {
    ...
    $_SESSION['message'] = "err:Please pick pending garments before submit new one";
...
if (empty($_POST['module']) || empty($_POST['style']) || empty($_POST['cw']) || empty($_POST['qty'])) {
    ...
    if ($stmt === false) {
        ...
        // Redirect to index.php after successful form submission with a success message
        $_SESSION['message'] = "ok:Successfully submitted the record";
        header("Location: index.php");
        exit();
    }
...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        // Add event listeners to 'cw' and 'qty' input fields
        document.getElementById("cw").addEventListener("input", validateInput);
        document.getElementById("qty").addEventListener("input", validateInput);
    });
    ...
    document.getElementById("submitBtn").addEventListener("click", function(event) {
        ...
        if (parseInt(cw) <= 0 || parseInt(qty) <= 0) {
            alert("Please enter valid values for Color Way and Quantity.");
        ...
        // Send a POST request to the server (same as before)
