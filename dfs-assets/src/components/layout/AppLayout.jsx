...
import { canDelete, canIssue, accessLabel } from '@/utils/accessControl'
...
const NAV = [
  ...
  { to: '/issuance',    label: 'Issuance',      icon: '⇗', section: 'Modules', issueOnly: true },
  ...
  { to: '/departments', label: 'Departments',   icon: '🏢', section: 'System', adminOnly: true },
]
        ...
        <nav className="flex-1 p-3 overflow-y-auto space-y-5">
          {sections.map(section => {
            const items = NAV.filter(n => {
              ...
              if (n.adminOnly && !canDelete(user)) return false
              // issueOnly → only super_admin + IT staff
              if (n.issueOnly && !canIssue(user)) return false
              return true
            })
          ...
          {sidebarOpen ? (
            <div className="flex items-center gap-2.5 px-2">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs font-mono text-brand-400 truncate">{accessLabel(user)}</div>
              </div>
              <button
                onClick={handleLogout}
                className="text-dark-500 hover:text-dark-200 text-sm transition-colors flex-shrink-0"
                title="Sign out"
              >⏻</button>
            </div>
          ) : (
            <button
              onClick={handleLogout}
              className="w-full flex justify-center py-1 text-dark-500 hover:text-dark-200 transition-colors"
              title="Sign out"
            >⏻</button>
          )}
        </div>
        ...
        <header className="h-14 bg-dark-850 border-b border-dark-700 flex items-center px-5 gap-4 flex-shrink-0">
          ...
          <div className="flex items-center gap-2 pl-4 border-l border-dark-700">
            ...
            {user?.role === 'super_admin' && (
              <span className="badge badge-red text-xs">Super Admin</span>
            )}
          </div>
        </header>
