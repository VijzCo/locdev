...
// Note: super_admin is created via /setup only. Admin cannot create another super_admin.
const ROLES = ['staff', 'viewer']
      ...
      <Modal open={!!modal} onClose={() => setModal(null)} title={modal === 'add' ? 'Add New User' : 'Edit User'}>
        ...
        <div className="bg-dark-900 border border-dark-700 rounded-lg p-3 mt-1 mb-2">
          ...
          <div className="text-xs text-dark-400 space-y-0.5">
            <div><span className="text-red-400">Super Admin</span> — You only. Full access + delete. Created via /setup.</div>
            <div><span className="text-blue-400">Staff (IT)</span> — See all depts. Create, edit, issue. No delete.</div>
            <div><span className="text-teal-400">Viewer</span> — Read-only. Own department only.</div>
          </div>
        </div>
